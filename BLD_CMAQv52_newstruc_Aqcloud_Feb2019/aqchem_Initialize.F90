...
!kf from aqchem_Rates to aqchem_Initialize 
...
!KF      REAL( kind=dp )FUNCTION KMTB ( HL, DH, ACC, DG, MW )
            IMPLICIT NONE
 
!KF            REAL( kind=dp ) HL, DH   ! Henry's Law coefficient, heat of dissolution
            REAL( kind=dp ) ACC      ! accommodation coefficient (unitless)
            REAL( kind=dp ) DG       ! gas molecular diffusion coef (m2/s)
            REAL( kind=dp ) MW       ! molecular weight (g/mol)
...
!kf      SUBROUTINE Initialize ( TEMP2, PRES_PA, TAUCLD, PRCRATE,  &
!kf         WCAVG, WTAVG, AIRM, ALFA0, ALFA3, GAS, AEROSOL, CTHK1, DARK )
...
! Set initial dynamic concentrations based on input gas and aerosol concentrations 
      ...
      VAR( ind_G_ISOPND )   = GAS( LISOPND )
      VAR( ind_G_ISOPNB )   = GAS( LISOPNB )
      VAR( ind_G_MVKN )     = GAS( LMVKN )
      VAR( ind_G_MACRN )    = GAS( LMACRN )
      VAR( ind_G_DHDN )     = GAS( LDHDN )
      VAR( ind_G_IMONIT )   = GAS( LIMONIT )
      VAR( ind_G_UTONIT )   = GAS( LUTONIT )
      VAR( ind_G_UTONIN )   = GAS( LUTONIN )
      VAR( ind_G_TONIT )    = GAS( LTONIT )
      VAR( ind_G_TONIN )    = GAS( LTONIN )
      VAR( ind_G_TONIH )    = GAS( LTONIH )
      VAR( ind_G_HONIT )    = GAS( LHONIT )
      VAR( ind_G_INHEDB )   = GAS( LINHED ) + GAS( LINHEB )
      VAR( ind_G_IHPN )     = GAS( LIHPN )
      VAR( ind_G_IHDPN )    = GAS( LIHDPN )
      VAR( ind_G_IDHPN )    = GAS( LIDHPN )
      VAR( ind_G_R4N )      = GAS( LR4N )
      VAR( ind_G_R4NO )     = GAS( LR4NO )
      VAR( ind_G_INPD )     = GAS( LINPD )
      VAR( ind_G_INPB )     = GAS( LINPB )
      VAR( ind_G_IHND )     = GAS( LIHND )
      VAR( ind_G_IHNB )     = GAS( LIHNB )
      VAR( ind_G_ISNP )     = GAS( LISNP )

      VAR( ind_L_ISOPND )    = AEROSOL( LCISOPND, ACC ) 
      VAR( ind_L_ISOPNB )    = AEROSOL( LCISOPNB, ACC ) 
      VAR( ind_L_MVKN )      = AEROSOL( LCMVKN, ACC ) 
      VAR( ind_L_MACRN )     = AEROSOL( LCMACRN, ACC ) 
      VAR( ind_L_DHDN )      = AEROSOL( LCDHDN, ACC ) 
      VAR( ind_L_IMONIT )    = AEROSOL( LCIMONIT, ACC ) 
      VAR( ind_L_UTONIT )    = AEROSOL( LCUTONIT, ACC ) 
      VAR( ind_L_UTONIN )    = AEROSOL( LCUTONIN, ACC ) 
      VAR( ind_L_TONIT )     = AEROSOL( LCTONIT, ACC ) 
      VAR( ind_L_TONIN )     = AEROSOL( LCTONIN, ACC ) 
      VAR( ind_L_TONIH )     = AEROSOL( LCTONIH, ACC ) 
      VAR( ind_L_HONIT )     = AEROSOL( LCHONIT, ACC ) 
      VAR( ind_L_INHEDB )    = AEROSOL( LCINHE, ACC ) 
      VAR( ind_L_IHPN )      = AEROSOL( LCIHPN, ACC ) 
      VAR( ind_L_IHDPN )     = AEROSOL( LCIHDPN, ACC ) 
      VAR( ind_L_IDHPN )     = AEROSOL( LCIDHPN, ACC ) 
      VAR( ind_L_R4N )       = AEROSOL( LCR4N, ACC ) 
      VAR( ind_L_R4NO )      = AEROSOL( LCR4NO, ACC ) 
      VAR( ind_L_INPD )      = AEROSOL( LCINPD, ACC ) 
      VAR( ind_L_INPB )      = AEROSOL( LCINPB, ACC ) 
      VAR( ind_L_IHND )      = AEROSOL( LCIHND, ACC ) 
      VAR( ind_L_IHNB )      = AEROSOL( LCIHNB, ACC ) 
      VAR( ind_L_ISNP )      = AEROSOL( LCISNP, ACC ) 
      VAR( ind_L_MTHYD )     = AEROSOL( LCMTHYD, ACC ) 
...
!  Solve for [OH-] with quadratic formula and use Kw to get initial [H+]
  ...
  RCONST(108) = ((KMTF(0.02D0,1.D-5,147.13D0)))
  RCONST(109) = ((KMTF(0.02D0,1.D-5,147.13D0)))
  RCONST(110) = ((KMTF(0.02D0,1.D-5,149.1D0)))
  RCONST(111) = ((KMTF(0.02D0,1.D-5,149.1D0)))
  RCONST(112) = ((KMTF(0.02D0,1.D-5,226.7D0)))
  RCONST(113) = ((KMTF(0.02D0,1.D-5,181.1D0)))
  RCONST(114) = ((KMTF(0.02D0,1.D-5,215.24D0)))
  RCONST(115) = ((KMTF(0.02D0,1.D-5,215.24D0)))
  RCONST(116) = ((KMTF(0.02D0,1.D-5,215.24D0)))
  RCONST(117) = ((KMTF(0.02D0,1.D-5,215.24D0)))
  RCONST(118) = ((KMTF(0.02D0,1.D-5,231.24D0)))
  RCONST(119) = ((KMTF(0.02D0,1.D-5,264.0D0)))
  RCONST(120) = ((KMTF(0.02D0,1.D-5,163.08D0)))
  RCONST(121) = ((KMTF(0.02D0,1.D-5,179.07D0)))
  RCONST(122) = ((KMTF(0.02D0,1.D-5,213.07D0)))
  RCONST(123) = ((KMTF(0.02D0,1.D-5,197.08D0)))
  RCONST(124) = ((KMTF(0.02D0,1.D-5,149.05D0)))
  RCONST(125) = ((KMTF(0.02D0,1.D-5,165.05D0)))
  RCONST(126) = ((KMTF(0.02D0,1.D-5,163.1D0)))
  RCONST(127) = ((KMTF(0.02D0,1.D-5,163.1D0)))
  RCONST(128) = ((KMTF(0.02D0,1.D-5,147.1D0)))
  RCONST(129) = ((KMTF(0.02D0,1.D-5,147.1D0)))
  RCONST(130) = ((KMTF(0.02D0,1.D-5,197.14D0)))
  RCONST(131) = ((KMTB(ISOPNDH,0.02D0,1.D-5,147.13D0)))
  RCONST(132) = ((KMTB(ISOPNBH,0.02D0,1.D-5,147.13D0)))
  RCONST(133) = ((KMTB(MVKNH,0.02D0,1.D-5,149.1D0)))
  RCONST(134) = ((KMTB(MACRNH,0.02D0,1.D-5,149.1D0)))
  RCONST(135) = ((KMTB(DHDNH,0.02D0,1.D-5,226.7D0)))
  RCONST(136) = ((KMTB(IMONITH,0.02D0,1.D-5,181.1D0)))
  RCONST(137) = ((KMTB(UTONITH,0.02D0,1.D-5,215.24D0)))
  RCONST(138) = ((KMTB(UTONINH,0.02D0,1.D-5,215.24D0)))
  RCONST(139) = ((KMTB(TONITH,0.02D0,1.D-5,215.24D0)))
  RCONST(140) = ((KMTB(TONINH,0.02D0,1.D-5,215.24D0)))
  RCONST(141) = ((KMTB(TONIHH,0.02D0,1.D-5,231.24D0)))
  RCONST(142) = ((KMTB(HONITH,0.02D0,1.D-5,264.0D0)))
  RCONST(143) = ((KMTB(INHEDBH,0.02D0,1.D-5,163.08D0)))
  RCONST(144) = ((KMTB(IHPNH,0.02D0,1.D-5,179.07D0)))
  RCONST(145) = ((KMTB(IHDPNH,0.02D0,1.D-5,213.07D0)))
  RCONST(146) = ((KMTB(IDHPNH,0.02D0,1.D-5,197.08D0)))
  RCONST(147) = ((KMTB(R4NH,0.02D0,1.D-5,149.05D0)))
  RCONST(148) = ((KMTB(R4NOH,0.02D0,1.D-5,165.05D0)))
  RCONST(149) = ((KMTB(INPDH,0.02D0,1.D-5,163.1D0)))
  RCONST(150) = ((KMTB(INPBH,0.02D0,1.D-5,163.1D0)))
  RCONST(151) = ((KMTB(IHNDH,0.02D0,1.D-5,147.1D0)))
  RCONST(152) = ((KMTB(IHNBH,0.02D0,1.D-5,147.1D0)))
  RCONST(153) = ((KMTB(ISNPH,0.02D0,1.D-5,197.14D0)))    
  RCONST(177) = (WETFAC_KPP)
  RCONST(178) = (WETFAC_KPP)
  RCONST(179) = (WETFAC_KPP)
  RCONST(180) = (WETFAC_KPP)
  RCONST(181) = (WETFAC_KPP)
  RCONST(182) = (WETFAC_KPP)
  RCONST(183) = (WETFAC_KPP)
  RCONST(184) = (WETFAC_KPP)
  RCONST(185) = (WETFAC_KPP)
  RCONST(186) = (WETFAC_KPP)
  RCONST(187) = (WETFAC_KPP)
  RCONST(188) = (WETFAC_KPP)
  RCONST(189) = (WETFAC_KPP)
