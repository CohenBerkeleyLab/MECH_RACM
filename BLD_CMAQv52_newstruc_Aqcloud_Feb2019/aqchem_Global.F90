...
!
      ...
      REAL( kind=dp ) :: ISOPNDH       ! ISOPND H Law coef - from HLCONST  
      REAL( kind=dp ) :: ISOPNBH       ! ISOPNB H Law coef - from HLCONST  
      REAL( kind=dp ) :: MVKNH         ! MVKN H Law coef - from HLCONST  
      REAL( kind=dp ) :: MACRNH        ! MACRN H Law coef - from HLCONST  
      REAL( kind=dp ) :: DHDNH         ! DHDN H Law coef - from HLCONST  
      REAL( kind=dp ) :: IMONITH       ! IMONIT H Law coef - from HLCONST  
      REAL( kind=dp ) :: UTONITH       ! UTONIT H Law coef - from HLCONST  
      REAL( kind=dp ) :: UTONINH       ! UTONIN H Law coef - from HLCONST  
      REAL( kind=dp ) :: TONITH        ! TONIT H Law coef - from HLCONST  
      REAL( kind=dp ) :: TONINH        ! TONIN H Law coef - from HLCONST  
      REAL( kind=dp ) :: TONIHH        ! TONIH H Law coef - from HLCONST  
      REAL( kind=dp ) :: HONITH        ! HONIT H Law coef - from HLCONST  
      REAL( kind=dp ) :: INHEDBH       ! INHED, INHEB H Law coef - from HLCONST  
      REAL( kind=dp ) :: IHPNH         ! IHPN H Law coef - from HLCONST  
      REAL( kind=dp ) :: IHDPNH        ! IHDPN H Law coef - from HLCONST  
      REAL( kind=dp ) :: IDHPNH        ! IDHPN H Law coef - from HLCONST  
      REAL( kind=dp ) :: R4NH          ! R4N H Law coef - from HLCONST  
      REAL( kind=dp ) :: R4NOH         ! R4NO H Law coef - from HLCONST  
      REAL( kind=dp ) :: INPDH         ! INPD H Law coef - from HLCONST  
      REAL( kind=dp ) :: INPBH         ! INPB H Law coef - from HLCONST  
      REAL( kind=dp ) :: IHNDH         ! IHND H Law coef - from HLCONST  
      REAL( kind=dp ) :: IHNBH         ! IHNB H Law coef - from HLCONST  
      REAL( kind=dp ) :: ISNPH         ! ISNP H Law coef - from HLCONST  
